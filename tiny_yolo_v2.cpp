...
int main(int argc, char* argv[])
{
    ...
    std::string model_path= "tiny_yolov2/Model.onnx";
    ...
    stbi_uc * img_data = stbi_load("yolo004.jpg", &img_sizex, &img_sizey, &img_channels, STBI_default);
    struct S_Pixel
    {
    ...
    //Config: label txt
    std::string filename("VOC_pascal_classes.txt");
    ...
    //hoaphan OrtReleaseAllocator(allocator);
    ...
    std::map<float,int> result; //Output for classification
        ...
        assert(is_tensor);
        //assert(g_ort->IsTensor(input_tensor));
      //hoaphan OrtReleaseAllocatorInfo(allocator_info);
